...
class ConfiguratorApp(tk.Tk):
    ...
    def show_lines(self, lines):
        '''Display lines in the editor area.'''
        ...
        for line in lines:
            ...
            entry.insert(0, line.rstrip('\n'))
            entry.pack(fill='x', padx=2, pady=1)
            self.modified_lines.append(entry)
    ...
    def apply_keyword_filter(self):
        ...
        for line in self.base_lines:
            match = True
            if keywords:
                match = any(kw.lower() in line.lower() for kw in keywords)
            if filter_text:
                match = match and (filter_text in line.lower())
            if match:
                filtered.append(line)
        self.show_lines(filtered)
    ...
    def save_base_config(self):
        ...
        try:
            content = '\n'.join(entry.get() for entry in self.modified_lines)
    ...
    def update_flash_card_keywords(self):
        ...
        if keywords:
            ...
            for kw in keywords:
                tk.Label(self.keywords_frame, text=kw, font=('Arial', 10), fg='black', anchor='w').pack(anchor='w')
        else:
